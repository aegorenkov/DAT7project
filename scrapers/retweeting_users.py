...
def scrape_retweeting_user(rate, timer):
    if timer.ready():
        retweeting_user_cache = TwitterUserCache(directory + r'/retweeting_users/')
        retweeter_list = read_csv(r'data\retweeter_list.csv')
        user_ids = [user for user in retweeter_list.user_id if not retweeting_user_cache.has_user(user)]
        if user_ids:
            user = TwitterUser(user_ids[0], retweeting_user_cache)
            user.load()
            user.save()
